...
void BestFirstSearch::setProblem(Problem* problem)
{
    m_problem = problem;
    m_path = std::make_unique<Grid::Path>(m_problem->getWorld());
}

bool BestFirstSearch::solve()
{
    ...
    while (!q.empty())
    {
        ...
        for (const auto& act : world->getActions())
        {
            ...
            int c2g = x->c2g + m_problem->getHeuristic(x->wp); // get new cost to go
            Point* dp = isVisited(xp);
            if (!dp)
            {
                visited.push_back(std::unique_ptr<Point>(new Point{xp, c2g}));             
                m_path->addWayPoint(Grid::WayPoint(x->wp, xp, act, Grid::EntryType::EXPLORE));
                q.push(visited.back().get()); // get the latest added
            }
            else // is visited, compare c2g
            {
                if (dp->c2g > c2g)
                {
...
void BestFirstSearch::SetFinalPathBetween(Grid::Point startPoint, Grid::Point endPoint)
{
...
Grid::Path* BestFirstSearch::getSolution() 
{
    return m_path.get();
}
